#!/usr/bin/env python3
"""
Test script for ${{ values.serviceName }} MCP integration.

This script tests the connection and functionality of the ${{ values.mcpType }} MCP server
...
def test_database_connection():
    ...
    print(f"\n🔍 Testing ${{ values.mcpType }} database connection...")
    ...
    try:
        ...
        result = client.execute_sql("SELECT @@VERSION as sql_server_version")
        {% endif %}
...
def test_table_operations():
    ...
    print(f"\n🔍 Testing ${{ values.mcpType }} table operations...")
    ...
    try:
        ...
        {% endif %}
        {% if values.mcpType == "ms-sql" %}
        tables = client.get_tables(schema_name="dbo")
        schema_name = "dbo"
        {% endif %}
...
def test_resource_access():
    ...
    print(f"\n🔍 Testing ${{ values.mcpType }} resource access...")
    ...
    try:
        ...
        if tables and len(tables) > 0:
            ...
            if test_table:
                ...
                {% endif %}
                {% if values.mcpType == "ms-sql" %}
                data = client.get_table_data(test_table, schema_name="dbo")
                {% endif %}
...
def test_error_handling():
    ...
    try:
        ...
        try:
            ...
            {% endif %}
            {% if values.mcpType == "ms-sql" %}
            result = client.get_table_schemas(["nonexistent_table_12345"], "dbo")
            {% endif %}
...
def print_configuration_help():
    ...
    print(f"\nMCP Server Type: ${{ values.mcpType }}")
    {% if values.mcpType == "postgres" %}
    ...
    print("Required environment variables:")
    print("- SECRET_ID (for AWS Secrets Manager) OR") 
    print("- MSSQL_HOST, MSSQL_USER, MSSQL_PASSWORD, MSSQL_DBNAME (for direct connection)")
    {% endif %}
...
def main():
    ...
    print("🚀 Starting ${{ values.serviceName }} MCP Integration Tests")
    print("=" * 60)
