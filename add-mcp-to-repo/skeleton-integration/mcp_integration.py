"""
Production MCP Integration Client for ${{ values.serviceName }}

This module provides a robust interface to connect your ML service 
to the production ${{ values.mcpType }} MCP server from bayer-int/mcp-servers.
...
class MCPClient:
    """Production MCP client for ${{ values.mcpType }} server integration."""
    ...
    def execute_sql(self, query: str) -> Union[str, Dict[str, Any]]:
        """
        Execute SQL query on ${{ values.mcpType }} database.
    ...
    def get_tables(self, schema_name: str = "dbo") -> List[Dict[str, Any]]:
        """
        Get MS SQL tables with descriptions.
    ...
    def get_table_schemas(
        ...
        """
        Get detailed MS SQL table schema information.
    ...
    def get_table_data(
        ...
        """
        Get sample data from MS SQL table.
        ...
        Returns:
            Table data as formatted string
        """
        resource_uri = f"mssql://{table_name}/data"
        return self._run_async(self._read_resource_async(resource_uri))
    {% endif %}
...
class MLServiceMCPIntegration:
    ...
    def get_training_data(
        self, 
        table_name: str = "training_data",
        where_clause: str = "active = 1",
        schema_name: str = "dbo"
    ) -> Dict[str, Any]:
        """Get training data for ML model."""
        query = f"SELECT TOP 10000 * FROM {schema_name}.{table_name}"
        if where_clause:
            query += f" WHERE {where_clause}"
        
        return self.mcp.execute_sql(query)
    
    def get_feature_metadata(self, schema_name: str = "dbo") -> Dict[str, Any]:
        """Get feature metadata for ML pipeline."""
        tables = self.mcp.get_tables(schema_name=schema_name)
        if tables:
            table_names = [t.get("name") for t in tables if t.get("name")]
        ...
        try:
            ...
            for table in tables[:5]:  # Check first 5 tables
                ...
                if table_name:
                    ...
                    {% endif %}
                    {% if values.mcpType == "ms-sql" %}
                    schema = self.mcp.get_table_schemas([table_name], "dbo")
                    {% endif %}
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "mcp_server_type": "${{ values.mcpType }}"
            }
        except Exception as e:
            return {
                "status": "unhealthy",
...
class ${{ values.serviceName | title }}WithMCP:
...
if __name__ == "__main__":
    ...
    if health.get("status") == "healthy":
        try:
            ...
            {% endif %}
            {% if values.mcpType == "ms-sql" %}
            tables = integration.mcp.get_tables("dbo")
            print(f"Available tables: {len(tables) if tables else 0}")
            {% endif %}
